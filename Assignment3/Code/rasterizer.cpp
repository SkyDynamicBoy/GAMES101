...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
	...
	auto v = t.toVector4();
	float up = std::floor(std::max({v[0][1],v[1][1],v[2][1]}));
	float down = std::floor(std::min({v[0][1],v[1][1],v[2][1]}));
	float left = std::floor(std::min({v[0][0],v[1][0],v[2][0]}));
	float right = std::floor(std::max({v[0][0],v[1][0],v[2][0]}));
	for (float y = up; y >= down; y--)
	{
